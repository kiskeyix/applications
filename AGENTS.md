...
- `share/claude/hooks/` — Claude Code hooks (`session-start.sh`, `notify-stop.sh`, `check-claude-version.sh`), symlinked to `~/.claude/hooks` by `update-host`; `share/claude/commands/` — custom slash commands (`claude-update-check.md`), symlinked to `~/.claude/commands`; `share/claude/AGENTS.md` is symlinked the same way but lands as `~/.claude/CLAUDE.md` — general, cross-project working principles, not project-specific facts (those stay in a repo's own file). The name has to be `CLAUDE.md`: Claude Code hardcodes the user-scope path and never reads `~/.claude/AGENTS.md`, and its `AGENTS.md` support is a project-scoped plugin that is off by default. `share/claude/settings.json.example` is merged (not symlinked) into `~/.claude/settings.json` (or `$CLAUDE_CONFIG_DIR/settings.json`) since Claude Code writes into that file at runtime. The repo's own `.claude/hooks` and `.claude/commands` are symlinks back into `share/claude/`, so this repo picks up the same hooks and commands when Claude Code works on itself, and `.claude/CLAUDE.md` is a symlink to the root-level `AGENTS.md` above for the same hardcoded-name reason.
