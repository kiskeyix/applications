...
# Defines
#-------------------------------------------------------
# Last change $Revision: 1.1 $ - $Author: luigi $ - $Date: 2003-12-12 18:13:23 $
...
.TABLEDATA { background-color: #$color_TableBG; }
...
EOF
    ...
    if ($ShowHeader) {
        ...
        if ($LogoLink =~ "http://awstats.sourceforge.net") {
            ...
            # logo is relative to this site or
            # an absolute link.
            print "<td class=AWL width=450><a href=\"$LogoLink\" target=\"_newawstats\"><img src=\"$Logo\" border=0></a></td></tr>\n";
        }
...
sub Read_Ref_Data {
    ...
    foreach my $file ("browsers.pl","domains.pl","operating_systems.pl","robots.pl","search_engines.pl") {
        foreach my $dir ("/usr/share/awstats/db") {
            my $searchdir=$dir;
...
sub Read_Language_Data {
    my $FileLang="";
    foreach my $dir ("$DirLang","/usr/share/awstats/lang") {
        my $searchdir=$dir;
        if (($searchdir ne "") && (!($searchdir =~ /\/$/)) && (!($searchdir =~ /\\$/)) ) { $searchdir .= "/"; }
