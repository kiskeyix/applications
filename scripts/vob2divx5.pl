...
Step I:
vob2divx5.pl /path/to/vobs sample
...
{       $vobpath = $ARGV[0];
	...
	for ($i = 0; $i <=6; $i ++)
	{
		make_sample("-x vob -y divx5 -a ".$i." ", "audio_sample._-a_".$i."_", $audiosample_length);
	}
	for ($i = 0; $i <= 80; $i += 16)
	{	for ($j = 0; $j <= 8; $j += 8)
		{	for ($B = 0; $B <= 4; $B ++)
			{       make_sample(" -j ".$i.",".$j." -B ".$B.",0 -x vob -y divx5 ", "video_s._-j_".$i.",".$j."_-B_".$B.",0", $videosample_length);
			}
		}
	}
	$elapsed=time-$begin_time;
...
#	$optionen = $ARGV[1];
	...
	{	if (! -e "tmp/1-".$datei.".done")
		{	print("Encode: ".$datei."\n");
			...
			{
				$sys = "transcode -i ".$vobpath."/".$datei." ".$params." -s ".$audio_rescale." -w ".$bitrate.",".$keyframes." -b ".$audio_bitrate." -x vob -y divx5 -R 1 -t ".$keyframes.",tmp/1-".$datei."-";
				print($sys."\n");
				system("nice -".$nice." ".$sys);
				#$exit_value  = $? >> 8;
				#print "\n\n--->>".$exit_value."<<---\n\n\n";
		...
		{	print("Encode: ".$datei."\n");
			...
			{
				$sys = "transcode -i ".$vobpath."/".$datei." ".$params." -s ".$audio_rescale." -w ".$bitrate.",".$keyframes." -b ".$audio_bitrate." -x vob -y divx5 -R 2 -t ".$keyframes.",tmp/2-".$datei."-";
				print($sys."\n");
				system("nice -".$nice." ".$sys);
