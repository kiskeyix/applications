...
describe :update_host do
  ...
  describe "verify_symlinks" do
    ...
    it "warns about a dead ~/.claude/AGENTS.md symlink once CLAUDE.md exists" do
      Dir.mktmpdir do |home|
        ...
        _(out.string).must_include "warn  ~/.claude/AGENTS.md is a dead symlink"
      end
    end

    it "does not warn about AGENTS.md when CLAUDE.md is missing or it isn't a symlink" do
      Dir.mktmpdir do |home|
        FileUtils.mkdir_p(File.join(home, ".claude"))
        File.write(File.join(home, ".claude", "AGENTS.md"), "a real file the user wrote")

        out = StringIO.new
        v = UpdateHost::Verifier.new(io: out)
        UpdateHost.verify_symlinks(v, home: home, repo_root: UpdateHost::REPO_ROOT)

        _(out.string).wont_include "dead symlink"
      end
    end
  end

  describe "retire_stale_agents_md" do
    it "removes a dead AGENTS.md symlink once CLAUDE.md exists" do
      Dir.mktmpdir do |home|
        FileUtils.mkdir_p(File.join(home, ".claude"))
        ...
        UpdateHost.retire_stale_agents_md(home: home)

        _(File.symlink?(agents_md)).must_equal false
        _(File.exist?(agents_md)).must_equal false
        _(File.symlink?(claude_md)).must_equal true
      end
    end

    it "leaves a real (non-symlink) AGENTS.md alone" do
      Dir.mktmpdir do |home|
        FileUtils.mkdir_p(File.join(home, ".claude"))
        agents_md = File.join(home, ".claude", "AGENTS.md")
        File.write(agents_md, "a real file the user wrote")
        File.symlink(File.join(UpdateHost::REPO_ROOT, "share", "claude", "AGENTS.md"), File.join(home, ".claude", "CLAUDE.md"))

        UpdateHost.retire_stale_agents_md(home: home)

        _(File.exist?(agents_md)).must_equal true
        _(File.symlink?(agents_md)).must_equal false
      end
    end

    it "does nothing when CLAUDE.md doesn't exist yet" do
      Dir.mktmpdir do |home|
        FileUtils.mkdir_p(File.join(home, ".claude"))
        agents_md = File.join(home, ".claude", "AGENTS.md")
        File.symlink(File.join(UpdateHost::REPO_ROOT, "share", "claude", "AGENTS.md"), agents_md)
        ...
        _(File.symlink?(agents_md)).must_equal true
      end
    end
  end
